...
from utils.domain_grl import GradientScalarLayer
...
class DAClassify(nn.Module):
    # Classification head, i.e. x(b,c1,20,20) to x(b,c2)
    def __init__(self, c1, c2):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv1 = nn.Conv2d(c1,128,1,1)
        self.act = nn.ReLU()
        self.conv = nn.Conv2d(128,c2,1,1)
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        x = self.pool(x)
        return x.view(x.size(0), -1)  # 展平为 [batch_size, output_dim]
...
#         super().__init__()
#         self.conv1 = nn.Conv2d(c1, 128, kernel_size=3, stride=1, padding=1)
...
#         self.act2 = nn.ReLU()
#
#         self.pool = nn.AdaptiveAvgPool2d((1, 1))  # 全局平均池化，输出变成 [B, 64, 1, 1]
#         self.fc = nn.Linear(64, c2)               # 全连接层，输入为64通道
#
#     def forward(self, x):
#         x = self.act1(self.bn1(self.conv1(x)))    # [B, 128, H, W]
#         x = self.act2(self.bn2(self.conv2(x)))    # [B, 64, H, W]
...
#         self.classifier = nn.Conv2d(c1, c2, kernel_size=1)
#
#     def forward(self, x):
