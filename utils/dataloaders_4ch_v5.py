...
from utils.augmentations_4ch_v5 import (
    Albumentations,
    augment_hsv,
...
class LoadImagesAndLabels(Dataset):
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=""):
        ...
        for _ in range(n):
            im = cv2.imread(random.choice(self.im_files), cv2.IMREAD_UNCHANGED)  # 支持.tiff的四通道
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # HSV color-space
            if img.shape[2] >= 3:  # only apply HSV on first 3 channels (RGB)
                img[:, :, :3] = augment_hsv(img[:, :, :3], hgain=hyp["hsv_h"], sgain=hyp["hsv_s"], vgain=hyp["hsv_v"])
        ...
        # Convert
        # Convert channel order and shape
        if img.shape[2] == 4:
            ...
            raise ValueError(f"Unsupported image with shape {img.shape}, expected 3 or 4 channels.")
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im = cv2.imread(f, cv2.IMREAD_UNCHANGED)  # 支持 4 通道.tiff
                assert im is not None, f"Image Not Found {f}"
                assert im.ndim == 3 and im.shape[2] in [3, 4], f"Unsupported image shape {im.shape} for file {f}"
