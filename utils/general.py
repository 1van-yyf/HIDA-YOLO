...
#
#     # **定义允许的类别**
#     allowed_classes = {2, 3, 6, 8, 10, 14, 15}
#
#     # 参数检查
#     assert 0 <= conf_thres <= 1, f'Invalid Confidence threshold {conf_thres}, valid values are between 0.0 and 1.0'
#     assert 0 <= iou_thres <= 1, f'Invalid IoU {iou_thres}, valid values are between 0.0 and 1.0'
...
#         x = torch.cat((box, conf, j.float(), mask), 1)[conf.view(-1) > conf_thres]
#
#         # **仅保留 allowed_classes 里的类别**
#         if allowed_classes is not None:
#             x = x[torch.isin(x[:, 5].long(), torch.tensor(list(allowed_classes), device=x.device))]
#
#         # **对每个类别筛选置信度最高的目标**
#         filtered_x = []
#         for cls_id in allowed_classes:
#             cls_instances = x[x[:, 5] == cls_id]  # 筛选当前类别的检测
#             if cls_instances.shape[0] > 0:
#                 best_instance = cls_instances[cls_instances[:, 4].argmax()]  # 取最高置信度的一条
#                 # **确保置信度足够高才保留**
#                 if best_instance[4] > conf_thres:
#                     filtered_x.append(best_instance)
#
#         if len(filtered_x) == 0:
#             continue
#         x = torch.stack(filtered_x)
#
#         # **Batched NMS**
